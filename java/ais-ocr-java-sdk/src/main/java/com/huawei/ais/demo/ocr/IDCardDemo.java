...
public class IDCardDemo {
	...
	// 身份证识别的使用示例函数
	//
	private static void idCardDemo() throws IOException {
		//
		...
		try {
			//
			// 2.构建访问身份证识别的使用示例函数服务需要的参数
			//
			String uri = "/v1.0/ocr/id-card";
			byte[] fileData = FileUtils.readFileToByteArray(new File("data/id-card-demo.png"));
			...
			//
			// 2.a 此项参数可选，可以指定获取信息为正面或者反面
			// side可选:"front", "back"或"",若为""或无此项参数输入则由算法自由判断，建议填写提高准确率
			//
			//json.put("side", "front");
			
			StringEntity stringEntity = new StringEntity(json.toJSONString(), "utf-8");
			
			// 3.传入身份证识别服务对应的uri参数, 传入身份证识别服务需要的参数，
