...
public class BarcodeDemo {
	...
	// 一维码识别的使用示例函数
	//
	private static void barcodeDemo() throws IOException {
		//
		...
		try {
			//
			// 2.构建访问一维码识别的使用示例函数服务需要的参数
			//
			String uri = "/v1.0/ocr/barcode";
			byte[] fileData = FileUtils.readFileToByteArray(new File("data/barcode_demo.jpg"));
			...
			// 3.传入一维码识别服务对应的uri参数, 传入一维码识别服务需要的参数，
