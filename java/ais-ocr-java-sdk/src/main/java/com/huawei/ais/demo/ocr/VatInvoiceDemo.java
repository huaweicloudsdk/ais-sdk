 ...
 *  增值税发票识别服务的使用示例类
 */
public class VatInvoiceDemo {
	//
	// 增值税发票识别的使用示例函数
	//
	private static void vatInvoiceDemo() throws IOException {
		//
		...
		try {
			//
			// 2.构建访问增值税发票识别服务需要的参数
			//
			String uri = "/v1.0/ocr/vat-invoice";
			byte[] fileData = FileUtils.readFileToByteArray(new File("data/vat-invoice-demo.jpg"));
			...
			// 3.传入增值税发票识别服务对应的uri参数, 传入增值税发票识别服务需要的参数，
