 ...
 *  海关单据识别服务的使用示例类
 */
public class CustomsFormEnDemo {
	//
	// 海关单据识别的使用示例函数
	//
	private static void customsFormEnDemo() throws IOException {
		//
		...
		try {
			...
			// 2.构建访问海关单据识别服务需要的参数
			//
			String uri = "/v1.0/ocr/action/ocr_form";
			byte[] fileData = FileUtils.readFileToByteArray(new File("data/customs-form-en-demo.jpg"));
			...
			// 3.传入海关单据识别服务对应的uri参数, 传入海关单据识别服务需要的参数，
