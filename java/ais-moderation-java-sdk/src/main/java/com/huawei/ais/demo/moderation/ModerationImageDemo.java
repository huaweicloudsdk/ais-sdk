 ...
 *  图像检测服务的使用示例类
 */
public class ModerationImageDemo {
	//
	// 图像检测服务的使用示例函数
	//
	private static void moderationImageDemo() throws IOException {
		//
		// 1. 在ClientContextUtils类中, 配置好访问图像检测服务的基本信息,
		// 然后，在此处生成对应的一个客户端连接对象
		//
		AisAccess service = new AisAccess(ClientContextUtils.getAuthInfo());

		//
		// 1.a 此处支持使用代理方式访问图像检测服务，用于不能直接访问华为云官网服务的情况, 例如，内网网络。
		...
		try {
			...
			// 3.传入图像检测服务对应的uri参数, 传入图像检测服务需要的参数，
